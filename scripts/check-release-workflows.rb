...
validate_step_contracts(release_steps, [
  ...
  { name: "Build release artifacts", keys: ["name", "env", "run"], values: { "env" => { "RELEASE_DATE" => "${{ steps.vars.outputs.date }}", "RELEASE_VERSION" => "${{ steps.vars.outputs.version }}" } }, run_sha256: "256017c308931b274da486157ab049c3666033683d8402d0e5f72de18e44bf23" },
  ...
  { name: "Build and push tunnel image", keys: ["name", "uses", "with"], values: { "uses" => "docker/build-push-action@10e90e3645eae34f1e60eeb005ba3a3d33f178e8", "with" => {
    "context" => ".",
    "file" => "docker/flowersec-tunnel/Dockerfile",
    "platforms" => "linux/amd64,linux/arm64",
    "push" => true,
    "sbom" => true,
    "tags" => "ghcr.io/${{ github.repository_owner }}/flowersec-tunnel:${{ steps.vars.outputs.version }}\nghcr.io/${{ github.repository_owner }}/flowersec-tunnel:latest\n",
    "build-args" => "VERSION=v${{ steps.vars.outputs.version }}\nCOMMIT=${{ github.sha }}\nDATE=${{ steps.vars.outputs.date }}\n",
  } } },
  { name: "Build and push proxy gateway image", keys: ["name", "uses", "with"], values: { "uses" => "docker/build-push-action@10e90e3645eae34f1e60eeb005ba3a3d33f178e8", "with" => {
    "context" => ".",
    "file" => "docker/flowersec-proxy-gateway/Dockerfile",
    "platforms" => "linux/amd64,linux/arm64",
    "push" => true,
    "sbom" => true,
    "tags" => "ghcr.io/${{ github.repository_owner }}/flowersec-proxy-gateway:${{ steps.vars.outputs.version }}\nghcr.io/${{ github.repository_owner }}/flowersec-proxy-gateway:latest\n",
    "build-args" => "VERSION=v${{ steps.vars.outputs.version }}\nCOMMIT=${{ github.sha }}\nDATE=${{ steps.vars.outputs.date }}\n",
  } } },
...
release_publication_steps = [
  ...
  "Build and push tunnel image",
  "Build and push proxy gateway image",
  "Publish npm package",
]
