...
在新 worktree 目录里开发。**每个新 worktree 创建后先执行一次 `make install-hooks`**，让仓库内 `.githooks/pre-commit` 生效；随后正常开发并跑门禁（以 `make check`/CI 为准）。准备合并前（**仅允许一次 rebase**），先把 `main` 的最新变更同步到 feature 分支并前置解决冲突：
...
- 所有开发 worktree 都必须安装仓库内 git hook：进入 worktree 后执行一次 `make install-hooks`。
- `pre-commit` hook 会自动执行 `make precommit`，提交前失败即中断 commit。
- `make precommit` 负责本地高价值快门禁，覆盖：
  - IDL/codegen 一致性：`gen-check`
  - 稳定性清单 / API 文档 / Go API 守护：`stability-check`
  - Go：`fmt-check`、`go vet`、`go test`
  - TS：缺依赖时自动 `npm ci --audit=false`，随后执行 `lint`、`build`、`test`、`verify:package`
- `pre-commit` 不能替代合并前门禁：准备合并前仍必须显式跑通一次 `make check`。
- `make check` 会覆盖：
...
- GitHub Release 正文必须包含“本次版本包含了什么”的特性说明，**不能只有 tag、资产和默认空正文**。
- Release 正文由仓库内生成器自动从“上一版 `flowersec-go/v*` tag 到当前 tag 的非 merge 提交标题”生成，因此提交标题必须简洁、可读、面向变更说明；`chore(release): prepare/bump ...` 这类纯发版 housekeeping 不应进入正文。
