...
Flowersec 2.3.4 is the published Swift package release.
...
```swift
.package(url: "https://github.com/floegence/flowersec.git", from: "2.3.4")
```
...
`connect(lease:options:)` establishes direct and tunneled WebSocket sessions on macOS and iOS. It validates TLS 1.3 for WSS and permits plaintext only for exact loopback direct candidates with the direct Flowersec v2 WebSocket subprotocol. WebSocket keeps Yamux internal to its adapter. Raw QUIC and WebTransport are explicitly unsupported because the Swift SDK does not implement those runtime adapters.
