...
final class ArtifactV2Tests: XCTestCase {
  ...
  func testSharedCandidateAndFSB2VectorsMatchByteForByte() throws {
    for item in try loadVectors().positive {
      ...
      for winner in item.winners {
        XCTAssertEqual(
          try AdmissionCodecV2.encodeFSB2(artifact: artifact, chosenCandidateID: winner.candidateID)
            .hexV2, winner.fsb2Hex, item.id)
      }
    }
  }
  ...
  func testSharedFSA2VectorsStrictlyDecode() throws {
    ...
    for item in vectors.fsa2 {
      let decoded = try AdmissionCodecV2.decodeFSA2(try Data(hexV2: item.frameHex))
    ...
    for item in vectors.negative where item.kind == "fsa2_hex" {
      XCTAssertThrowsError(try AdmissionCodecV2.decodeFSA2(try Data(hexV2: item.value)), item.id)
    }
  }
  ...
  func testFSA2RejectsEveryMalformedStatusAndReasonBoundary() throws {
    ...
    for frame in invalidFrames {
      XCTAssertThrowsError(try AdmissionCodecV2.decodeFSA2(try Data(hexV2: frame)), frame)
    }
    XCTAssertEqual(
      try AdmissionCodecV2.decodeFSA2(try Data(hexV2: "46534132020200086361706163697479")),
...
private struct ArtifactVectorsV2: Decodable {
  ...
  struct Positive: Decodable {
    ...
    let candidatesCanonicalJSON: String
    let candidateSetHashBase64URL: String
    let winners: [Winner]
    enum CodingKeys: String, CodingKey {
      case id, winners
      case artifactJSON = "artifact_json"
...
extension Data {
  ...
  fileprivate var hexV2: String { map { String(format: "%02x", $0) }.joined() }
  fileprivate var base64URLForTest: String {
