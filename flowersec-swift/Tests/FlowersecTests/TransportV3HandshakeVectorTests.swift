...
final class TransportV3HandshakeVectorTests: XCTestCase {
  func testHandshakeCodecAndKDFMatchSharedVectors() throws {
    ...
    for vector in fixture.vectors {
      let suite = try XCTUnwrap(TransportCipherSuiteV3(rawValue: vector.suite))
      try TransportV3Handshake.verifyVectorForTesting(
        TransportV3HandshakeVectorInput(
          id: vector.id,
          suite: suite,
          fsc3: try handshakeHex(vector.fsc3Hex),
  ...
  private func loadFixture() throws -> HandshakeVectorFile {
    let url = packageRoot().appendingPathComponent("testdata/transport_v3/handshake_vectors.json")
