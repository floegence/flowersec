...
protocol TransportV3CarrierStream: Sendable {
...
protocol TransportV3CarrierSession: Sendable {
  var chosenCarrier: CarrierKind { get }
  var capabilities: CarrierCapabilitiesV3 { get }
...
struct CarrierCapabilitiesV3: Equatable, Sendable {
  let reliableStreams: Bool
  let datagrams: Bool
  let migration: Bool
}

enum TransportV3CarrierError: Error, Equatable, Sendable {
...
enum TransportV3SessionError: Error, Equatable, Sendable {
...
struct TransportV3SessionDeadlines: Sendable {
  static let production = TransportV3SessionDeadlines(
...
struct TransportV3SessionConfig: Sendable {
  let role: SessionRoleV3
  let path: PathKind
  let channelID: String
  let sessionContractHash: Data
  let suite: TransportCipherSuiteV3
  let psk: Data
  ...
  var deadlines: TransportV3SessionDeadlines
...
struct TransportV3HandshakeMaterial: Sendable {
  let h3: Data
  let sessionPRK: Data
}

struct TransportV3HandshakeVectorInput {
  let id: String
  let suite: TransportCipherSuiteV3
  let fsc3: Data
  let clientInit: Data
...
enum TransportV3Handshake {
  static let controlPrefaceBytes = 16
  static let frameHeaderBytes = 12
  static let maxPayloadBytes = 8_192

  static func verifyVectorForTesting(_ vector: TransportV3HandshakeVectorInput) throws {
    func require(_ condition: Bool) throws {
      guard condition else { throw TransportV3SessionError.handshakeFailed }
    }

    try require(vector.fsc3 == controlPreface())
  ...
  ) async throws -> (any TransportV3CarrierStream, TransportV3HandshakeMaterial) {
  ...
  private static func validate(
    ...
    guard
      ...
      throw TransportV3SessionError.invalidConfiguration
    }
    switch config.path {
    case .direct:
      guard
      ...
      else { throw TransportV3SessionError.invalidConfiguration }
    case .tunnel:
      guard
  ...
  private static func clientHandshake(
    ...
    try await writeAll(fsc3, to: control)
  ...
  static func controlPreface() -> Data {
    ...
    data.replaceSubrange(0..<4, with: Data("FSC3".utf8))
    data[4] = 3
    data[5] = 1
    return data
  }
  ...
  static func readFrame(
    ...
    guard
      Data(header[0..<4]) == Data("FSH3".utf8),
      header[4] == 3,
  ...
  private static func decodeCanonical<Value: Codable>(
    ...
    guard raw.count >= frameHeaderBytes else { throw TransportV3SessionError.handshakeFailed }
  ...
  private static func decodePublicKey(
    ...
    case .aes256GCM:
      guard raw.first == 4 else { throw TransportV3SessionError.handshakeFailed }
...
private enum EphemeralKey {
  ...
  init(suite: TransportCipherSuiteV3) {
    switch suite {
    case .chacha20Poly1305:
  ...
  init(suite: TransportCipherSuiteV3, rawRepresentation: Data) throws {
