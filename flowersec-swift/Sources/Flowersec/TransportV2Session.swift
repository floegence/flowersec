...
actor TransportV2Session {
  ...
  private func encodeControl(_ type: InnerRecordTypeV2, payload: Data) throws -> Data {
...
private actor TransportV2CarrierWriter {
  ...
  func write(_ data: Data) async throws {
    guard !terminalCommitted else { throw TransportV2SessionError.closed }
    let previous = tail
  ...
  func writeTerminal(_ records: [Data]) async throws {
    ...
    let operation = Task {
      ...
      for record in records {
        try await TransportV2Handshake.writeAll(record, to: stream)
      }
