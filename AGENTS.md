...
- The exact-main final gate runs complete race as an exclusive first phase through `final-race-check`, then runs Go, TypeScript, Swift, and Rust through four bounded parallel lanes. Each phase has its own audited 595-second process-group hard stop; do not apply one 595-second limit to the complete serial composition. Exact-main evidence showed that sharing CPU and filesystem bandwidth with cold Swift and Rust builds pushed otherwise passing race shards beyond five minutes. Generation, package inventory, examples, cross-language interoperability, remote-runner, and evidence targets remain serial. Do not overlap the race phase with language build lanes, remove either stage boundary, or move a check between these groups without updating the Make-graph contract tests.
- The final-only `transportcheck` race shards are separate test processes with disjoint test selections and isolated temporary resources. On the audited 18-CPU final-gate host, distribute the discovered tests across 36 fine-grained shards and run them through nine dynamically refilled worker slots with `GOMAXPROCS=1` for every worker. With the current 140-test inventory, each shard owns at most four tests; start smaller tail shards before full shards, and release each completed slot immediately instead of waiting for a batch. Preserve the complete temporary shard directory on failure or external termination so exact logs, selections, and completion markers remain auditable. Two exact-main samples proved that the earlier 18-shard layout created 191-310 second long tails under final-lane contention even though the other four lanes completed. Preserve every test, race instrumentation, and the five-minute per-shard timeout; process concurrency and scheduler limits are not `go test -parallel`. Any future budget change requires a deterministic Make/runner contract test and exact-main evidence; do not infer it from an unchanged expensive rerun.
