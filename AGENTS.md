...
- The exact-main final gate runs complete race as an exclusive first phase through `final-race-check`, then runs Go, TypeScript, Swift, and Rust through four bounded parallel lanes. Exact-main evidence showed that sharing CPU and filesystem bandwidth with cold Swift and Rust builds pushed otherwise passing race shards beyond five minutes and prevented the 595-second gate from completing. Generation, package inventory, examples, cross-language interoperability, remote-runner, and evidence targets remain serial. Do not overlap the race phase with language build lanes or move a check between these groups without updating the Make-graph contract tests.
