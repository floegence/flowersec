...
- The exact-main final gate runs a bounded network preflight, then bounded serial package validation, then complete race as an exclusive phase through `final-race-check`, and finally Go, TypeScript, Swift, and Rust through four bounded parallel lanes. The race and language phases each have their own audited 595-second process-group hard stop; do not apply one 595-second limit to the complete serial composition. Package and publish dry-runs must fail before race begins and must not be repeated in language lanes. Exact-main evidence showed that sharing CPU and filesystem bandwidth with cold Swift and Rust builds pushed otherwise passing race shards beyond five minutes. Generation, examples, cross-language interoperability, remote-runner, and evidence targets remain serial. Do not overlap the race phase with language build lanes, remove a stage boundary, or move a check between these groups without updating the Make-graph contract tests.
- The final-only `transportcheck` race shards are separate test processes with disjoint test selections and isolated temporary resources. On the audited 18-CPU final-gate host, distribute the discovered tests across 45 fine-grained shards and run them through nine dynamically refilled worker slots with `GOMAXPROCS=1` for every worker. With the current 148-test inventory, each shard owns at most four tests; start full shards before smaller tail shards so the largest known work units receive the complete stage budget, and release each completed slot immediately instead of waiting for a batch. Preserve the complete temporary shard directory on failure or external termination so exact logs, selections, and completion markers remain auditable. Two exact-main samples proved that the earlier 18-shard layout created 191-310 second long tails under final-lane contention, and the 45-shard sparse-first layout deferred a four-test shard until the final wave and exhausted the 595-second stage budget after the other 44 shards passed. Preserve every test, race instrumentation, and the five-minute per-shard timeout; process concurrency and scheduler limits are not `go test -parallel`. Any future budget change requires a deterministic Make/runner contract test and exact-main evidence; do not infer it from an unchanged expensive rerun.
- Keep commands that share one language build directory serial inside their final lane. In particular, `swift-check` must invoke package/security/source checks, build, the coverage-producing test, and the coverage reader as ordered recursive Make recipe lines so they cannot overlap when the outer Make runs in parallel. Do not use target-scoped `.NOTPARALLEL`: the Xcode-provided GNU Make 3.81 treats it as a global serialization switch and prevents independent final lanes from running concurrently.
