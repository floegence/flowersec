...
- The final-only `transportcheck` race shards are separate test processes with disjoint test selections and isolated temporary resources. On the audited 18-CPU final-gate host, run all 18 shards through nine dynamically refilled worker slots and set `GOMAXPROCS=1` for every worker. A completed shard must release its slot immediately; do not wait for a whole batch. The measured nine-worker worst shard gives a two-wave upper bound of 535.678 seconds, below the 595-second final gate, while starting 18 workers together starves fixture I/O and other final lanes. Preserve all shards, race instrumentation, and the five-minute per-shard timeout; process concurrency and scheduler limits are not `go test -parallel`. Any future budget change requires a deterministic Make/runner contract test and exact-main evidence; do not infer it from an unchanged expensive rerun.
