...
- SwiftPM releases use root semantic-version tags with no prefix, such as `0.19.12`.
  - The root `Package.swift` must describe a buildable Swift package at that tag.
  - Downstream Swift packages should prefer version ranges such as `.package(url: "https://github.com/floegence/flowersec.git", from: "0.19.12")`.
