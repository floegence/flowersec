...
- The final-only `transportcheck` race shards are separate test processes with disjoint test selections and isolated temporary resources. On the audited 18-CPU final-gate host, start all 18 shards together and set `GOMAXPROCS=1` for every worker. This caps the race lane at 18 Go scheduler slots while avoiding a second batch whose measured lower bound cannot complete inside the 595-second final gate. Preserve all shards, race instrumentation, and the five-minute per-shard timeout; process concurrency and scheduler limits are not `go test -parallel`. Any future budget change requires a deterministic Make/runner contract test and exact-main evidence; do not infer it from an unchanged expensive rerun.
