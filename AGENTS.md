...
- The final-only `transportcheck` race shards are separate test processes with disjoint test selections and isolated temporary resources. On the audited 18-CPU final-gate host, run all 18 shards in two batches of nine processes and set `GOMAXPROCS=1` for every worker. This keeps the race lane active from the start while reserving half of the CPU budget for the other four final lanes. Preserve all shards, race instrumentation, and the five-minute per-shard timeout; process batching and scheduler limits are not `go test -parallel`. Any future budget change requires a deterministic Make/runner contract test and exact-main evidence; do not infer it from an unchanged expensive rerun.
