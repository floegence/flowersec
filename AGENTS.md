...
- The exact-main final gate runs its Go, complete race, TypeScript, Swift, and Rust lanes through the audited `final-integration-lanes` target. These five lanes may run in bounded parallel because they use read-only source inputs, concurrency-safe language caches, and separate test processes or build trees; generation, package inventory, examples, cross-language interoperability, remote-runner, and evidence targets remain serial. Keep complete race in its own lane so it starts with the other final checks. Do not move a check between these groups without updating the Make-graph contract tests.
