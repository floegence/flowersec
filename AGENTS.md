...
- Keep commands that share one language build directory serial inside their final lane. In particular, `swift-check` must retain its ordered prerequisites under a target-scoped `.NOTPARALLEL` declaration so package/security/source checks, build, the coverage-producing test, and the coverage reader cannot overlap when the outer Make runs in parallel.
