...
- The exact-main final gate runs its Go, TypeScript, Swift, and Rust lanes through the audited `final-integration-lanes` target. These four lanes may run in bounded parallel because they use separate build trees; generation, package inventory, examples, cross-language interoperability, remote-runner, and evidence targets remain serial. Do not move a check between these groups without updating the Make-graph contract tests.
