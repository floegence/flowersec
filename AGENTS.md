...
- The final-only `transportcheck` race shards are separate test processes with disjoint test selections and isolated temporary resources. Run all 18 shards concurrently on the frozen exact-main candidate so the gate does not serialize them into repeated batches. Preserve all shards, race instrumentation, and the five-minute per-shard timeout; this process-level concurrency is not `go test -parallel`.
