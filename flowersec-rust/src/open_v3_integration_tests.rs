use crate::protocol_v3::{OpenPayloadV3, decode_open_payload_v3, encode_open_payload_v3};
...
fn open_unicode_and_canonical_metadata_vectors() {
    ...
    for vector in fixture.positive {
        ...
        let metadata = vector
            ...
            .into_bytes();
        let encoded = encode_open_payload_v3(&OpenPayloadV3::new(
            1,
            [0; 32],
            kind.clone(),
            metadata.clone(),
        ...
        let decoded = decode_open_payload_v3(&encoded)
            .unwrap_or_else(|error| panic!("{}: {error}", vector.id));
    ...
    for vector in fixture.negative {
        ...
        if let Some(kind_hex) = vector.kind_utf8_hex {
            ...
            assert!(decode_open_payload_v3(&raw).is_err(), "{}", vector.id);
            continue;
        }
        let payload = OpenPayloadV3::new(1, [0; 32], vector.kind.unwrap_or_default(), metadata);
        assert!(encode_open_payload_v3(&payload).is_err(), "{}", vector.id);
    }
}
...
fn open_enforces_existing_metadata_limits_at_the_boundary() {
    ...
    for (kind, metadata) in valid {
        encode_open_payload_v3(&OpenPayloadV3::new(1, [0; 32], kind, metadata.into_bytes()))
    ...
    assert_eq!(decode_open_payload_v3(&encoded).unwrap().metadata(), b"{}");
}
