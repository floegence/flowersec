...
impl TestDatagramCarrierSession {
    fn wrap_control(&self, stream: Arc<dyn CarrierStreamV2>) -> Arc<dyn CarrierStreamV2> {
...
impl CarrierSessionV2 for TestDatagramCarrierSession {
    ...
    async fn receive_unreliable_message(&self) -> Result<Bytes, CarrierUnreliableMessageErrorV2> {
        self.incoming
            ...
            .recv()
            .await
            .ok_or(CarrierUnreliableMessageErrorV2::Closed)
    }
...
#[async_trait::async_trait]
impl CarrierStreamV2 for PostWriteGatedCarrierStream {
...
async fn bidirectional_open_data_fin_and_consecutive_rekey() {
    ...
    assert_eq!(format!("{opened:?}"), "EncryptedStreamV2 { <opaque> }");
    assert_eq!(
        format!("{:?}", incoming.stream()),
        "EncryptedStreamV2 { <opaque> }"
    );
...
async fn receive_rekey_commits_before_the_ack_is_exposed_to_unreliable_delivery() {
    ...
    let client_carrier: Arc<dyn CarrierSessionV2> = Arc::new(TestDatagramCarrierSession {
        inner: client_inner,
    ...
    let server_carrier: Arc<dyn CarrierSessionV2> = Arc::new(TestDatagramCarrierSession {
        inner: server_inner,
