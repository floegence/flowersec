    ...
    async fn rekey(&self) -> Result<(), SessionError> {
        rekey_v3(self).await.map_err(|error| {
            ...
            match public {
                ...
                | SessionError::ResourceExhausted => public,
...
async fn receive_unreliable_message_v3(
    ...
    loop {
        ...
        let plaintext = {
            let state = session.state.lock().await;
            if !unreliable_receive_epoch_is_committed_v3(state.recv_epoch, header.epoch) {
                continue;
            }
        ...
        let mut replay = session
            ...
            .lock()
            .expect("unreliable replay lock poisoned");
        record_unreliable_sequence_v3(&mut replay, header.epoch, header.sequence);
        touch_activity_v3(session);
...
fn record_unreliable_sequence_v3(
    replay: &mut HashMap<u32, ReplayWindowV3>,
    epoch: u32,
    sequence: u64,
) {
...
fn resource_exhausted(message: &'static str) -> io::Error {
    io::Error::new(io::ErrorKind::OutOfMemory, message)
}
fn next_session_transition_v3(current: u64) -> Option<u64> {
    (current != 0).then(|| current.wrapping_add(1))
}
fn expected_session_transition_v3(current: u64) -> Option<u64> {
...
async fn send_control_v3(
    ...
    write_control_locked_v3(session, kind, payload).await
}

async fn send_control_response_v3(
    session: &EncryptedSessionV3,
    kind: InnerRecordTypeV3,
    payload: &[u8],
) -> io::Result<bool> {
    ...
    write_control_locked_v3(session, kind, payload)
        .await
        .map(|()| true)
}

async fn send_control_cleanup_v3(
    session: &EncryptedSessionV3,
    kind: InnerRecordTypeV3,
    payload: &[u8],
) -> io::Result<()> {
    let _write = session.control_write.lock().await;
    if session.lifecycle() == SessionLifecycleV3::Closed
...
async fn write_control_locked_v3(
    session: &EncryptedSessionV3,
    kind: InnerRecordTypeV3,
    payload: &[u8],
) -> io::Result<()> {
    ...
    loop {
        ...
        let handled = match kind {
            InnerRecordTypeV3::Ping => {
                send_control_response_v3(&session, InnerRecordTypeV3::Pong, &payload)
                    .await
                    .map(|_| ())
            }
...
async fn prepare_rekey_v3(session: &Arc<SelfSession>) -> io::Result<PreparedRekeyV3> {
    ...
    let (next_epoch, next_roots) = {
        ...
        let next = state
            .send_epoch
            .checked_add(1)
            .ok_or_else(|| resource_exhausted("session epoch exhausted"))?;
        let roots = state
            .send_roots
...
async fn run_owned_rekey_v3(session: &SelfSession, prepared: PreparedRekeyV3) -> io::Result<()> {
    ...
    {
        Ok(Ok(())) => Ok(()),
        Ok(Err(_)) if session.is_closed() => Err(terminal_error_v3(session)),
        Ok(Err(error)) => Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("Flowersec v3 rekey completion failed: {error}"),
        )),
...
async fn commit_and_complete_rekey_v3(
    ...
    let next_transition = next_session_transition_v3(prepared.transition)
        .ok_or_else(|| resource_exhausted("rekey transition exhausted"))?;
    session
        .next_transition
        .compare_exchange(
            prepared.transition,
...
fn retain_receive_epoch_roots_v3(
    roots: &mut HashMap<u32, EpochRootsV3>,
    recv_epoch: u32,
    control_recv_epoch: u32,
) {
...
async fn write_goaway_locked_v3(session: &EncryptedSessionV3, reason: u16) -> io::Result<()> {
    if reason == 0 {
...
async fn send_goaway_v3(session: &EncryptedSessionV3, reason: u16) -> io::Result<()> {
    ...
    write_goaway_locked_v3(session, reason).await
}
                ...
                if session.begin_closing() {
                    ...
                    let _ = tokio::time::timeout(session.config.deadlines.close_flush, async {
                        ...
                        session
                            .control_terminal_committed
                            .store(true, Ordering::Release);
                        write_goaway_locked_v3(&session, IDLE_TIMEOUT_REASON_V3).await?;
                        Ok::<(), io::Error>(())
                    })
                    .await;
                    session
...
async fn commit_outbound_abandonment_v3(
    ...
    if let Err(error) =
        send_control_cleanup_v3(&session, InnerRecordTypeV3::StreamReset, &payload).await
    {
        if !session.is_closed() {
            fail_session_v3(&session, error);
        }
...
impl EncryptedStreamV3 {
    ...
    async fn finish_reset(self: Arc<Self>) {
        if let Some(session) = self.session.upgrade() {
            ...
            let result =
                match send_control_cleanup_v3(&session, InnerRecordTypeV3::StreamReset, &payload)
                    .await
                {
...
mod tests {
    ...
    fn receive_epoch_root_cleanup_preserves_control_current_and_pending_epochs() {
        ...
        retain_receive_epoch_roots_v3(&mut epochs, 2, 1);
        ...
        retain_receive_epoch_roots_v3(&mut caught_up, 3, 3);
    ...
    fn stream_key_update_ack_matches_the_shared_wire_vector() {
        ...
        let maximum =
            u64::from_str_radix(boundary["maximum_transition_id_hex"].as_str().unwrap(), 16)
                .unwrap();
        let next_after_maximum =
            u64::from_str_radix(boundary["next_after_maximum_hex"].as_str().unwrap(), 16).unwrap();
        assert_eq!(maximum, u64::MAX);
        assert_eq!(next_after_maximum, 0);
        assert_eq!(
            next_session_transition_v3(maximum),
            Some(next_after_maximum)
        );
        assert_eq!(next_session_transition_v3(next_after_maximum), None);
        assert_eq!(expected_session_transition_v3(maximum), None);
        assert_eq!(boundary["maximum_is_usable_once"], true);
    ...
    #[tokio::test]
    async fn maximum_session_transition_is_used_once_then_exhausts_with_goaway() {
        let (client_carrier, server_carrier) = memory_carrier_pair_v3_with_capacity(3);
        let (client, server) = tokio::join!(
            establish_session_v3_inner(
                client_carrier,
        ...
        let client = client.expect("client session");
        let server = server.expect("server session");
        client.next_transition.store(u64::MAX, Ordering::Release);
        server
            .recv_transition
            .store(u64::MAX - 1, Ordering::Release);

        assert_eq!(client.rekey().await, Ok(()));
        assert_eq!(client.next_transition.load(Ordering::Acquire), 0);
        assert_eq!(client.rekey().await, Err(SessionError::ResourceExhausted));
        tokio::time::timeout(Duration::from_secs(1), async {
            while !server.received_goaway.load(Ordering::Acquire) {
                tokio::task::yield_now().await;
            }
        })
        .await
        .expect("peer did not receive transition-exhaustion GOAWAY");
        assert_eq!(
            server
                .open_stream("after-transition-exhaustion", StreamMetadata::empty())
                .await
                .err(),
    ...
    #[tokio::test]
    async fn session_epoch_exhaustion_returns_resource_exhausted_with_goaway() {
        let (client_carrier, server_carrier) = memory_carrier_pair_v3_with_capacity(3);
        let (client, server) = tokio::join!(
            establish_session_v3_inner(
                client_carrier,
        ...
        let server = server.expect("server session");
        {
            ...
            let roots = state
                .send_roots
                .get(&0)
                .expect("client epoch zero roots")
                .clone();
            state.send_roots.insert(u32::MAX, roots);
            state.send_epoch = u32::MAX;
            state.control_send_sequence = 0;
        }
        {
            ...
            let roots = state
                .recv_roots
                .get(&0)
                .expect("server epoch zero roots")
                .clone();
            state.recv_roots.insert(u32::MAX, roots);
            state.recv_epoch = u32::MAX;
            state.control_recv_epoch = u32::MAX;
            state.control_recv_sequence = 0;
        }
        ...
        .await
        .expect("peer did not receive epoch-exhaustion GOAWAY");
        assert_eq!(
            server
                .open_stream("after-epoch-exhaustion", StreamMetadata::empty())
                .await
                .err(),
            Some(SessionError::GoingAway),
        );
    ...
    fn unreliable_replay_state_rejects_future_epochs_and_keeps_two_windows() {
        assert!(unreliable_receive_epoch_is_committed_v3(2, 2));
        assert!(unreliable_receive_epoch_is_committed_v3(2, 1));
        assert!(!unreliable_receive_epoch_is_committed_v3(2, 3));

        let mut replay = HashMap::new();
        for epoch in 0..4 {
            record_unreliable_sequence_v3(&mut replay, epoch, 1);
        }
