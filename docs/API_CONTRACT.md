...
The root exposes unversioned application names: `Artifact`, `ArtifactLease`, `Session`, `RpcPeer`, `ByteStream`, reconnect orchestration, `classifyConnectError(...)`, `classifySessionError(...)`, `RetryAction`, `ErrorRetryClassification`, typed `RpcResult<Response>`, `ConnectError`, and `SessionError`. `RpcResult<Response>` is a discriminated union: callers branch on `ok` before reading either a typed success `payload` or a bounded application `error`. A negotiated session may expose `UnreliableMessageChannel`; applications construct its opaque payload with `createUnreliableMessage(...)`, while invalid operations return `UnreliableMessageError`. Browser applications add `connectBrowserSession(...)`; Node.js applications add `connectNodeSession(...)` with carrier-neutral TLS options. Low-level carrier factories, capability descriptors, candidate diagnostics, wire contracts, and cryptographic state are not package exports. Wire policy constants retain `V2` where the suffix identifies a protocol contract rather than an application type.
