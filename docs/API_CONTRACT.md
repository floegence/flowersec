...
Across all four SDKs, an omitted public connection timeout uses the shared ten-second default from `stability/sdk_defaults.json`. The portable core is artifact/lease lifecycle, one-shot connection, authenticated sessions and reliable streams, outbound RPC call/notify, and recovery classification. SDK profiles add runtime and carrier capabilities; language conveniences improve syntax and typing without changing wire semantics.

| Capability layer | Contract | Go | TypeScript | Swift | Rust |
| --- | --- | :---: | :---: | :---: | :---: |
| `portable_core` | Artifact/lease, one-shot connect, session, reliable streams, RPC call/notify, recovery classification | Yes | Yes | Yes | Yes |
...
The protocol carrier set is not a promise that every SDK exposes every carrier. Profile differences are not part of the portable API.
...
- Connection: `flowersec.ConnectorOptions`, `flowersec.Connect(...)`, `flowersec.ConnectError`, and `flowersec.ConnectErrorCode`. Optional `ConnectorOptions.Handlers` snapshots inbound RPC registrations before session establishment.
- Session values: `flowersec.Session`, `flowersec.SessionTermination`, `flowersec.Metadata`, `flowersec.NewMetadata(...)`, `flowersec.ErrInvalidMetadata`, `flowersec.ByteStream`, `flowersec.IncomingStream`, and `flowersec.RPCPeer`.
...
- Session lifecycle: `flowersec.Session.RPC()`, `flowersec.Session.OpenStream(...)`, `flowersec.Session.AcceptStream(...)`, `flowersec.Session.Rekey(...)`, `flowersec.Session.ProbeLiveness(...)`, `flowersec.Session.Termination()`, `flowersec.Session.WaitTermination(...)`, `flowersec.Session.WaitClosed(...)`, and `flowersec.Session.Close()`.
...
- Connection outcomes: `flowersec.ConnectInvalidInput`, `flowersec.ConnectInvalidOptions`, `flowersec.ConnectCanceled`, `flowersec.ConnectExpired`, `flowersec.ConnectTimeout`, `flowersec.ConnectConnectionFailed`, compatibility aliases `flowersec.ConnectInvalid` and `flowersec.ConnectFailed`, `flowersec.ErrInvalidConnectorOptions`, and `flowersec.ErrConnectionFailed`.
...
The root exposes unversioned application names: `Artifact`, `ArtifactLease`, `ArtifactLeaseError`, `Session`, `RpcPeer`, `ByteStream`, `classifyConnectError(...)`, `classifySessionError(...)`, `RetryAction`, `ErrorRetryClassification`, typed `RpcResult<Response>`, `ConnectError`, and `SessionError`. `RpcResult<Response>` is a discriminated union: callers branch on `ok` before reading either a typed success `payload` or a bounded application `error`. `Session.waitClosed()` is the sole public termination waiting entrypoint. A negotiated session may expose `UnreliableMessageChannel`; applications construct its opaque payload with `createUnreliableMessage(...)`, while invalid operations return `UnreliableMessageError`. Browser applications add `connectBrowserSession(...)`; Node.js applications add `connectNodeSession(...)` with carrier-neutral TLS options. Both connectors use a shared ten-second connection timeout by default and accept `connectTimeoutMs` without exposing internal clock or candidate-cleanup controls. Low-level carrier factories, capability descriptors, candidate diagnostics, wire contracts, and cryptographic state are not package exports.
...
Applications `import Flowersec` from the `Flowersec` product. The public lifecycle is `parseArtifact(...)`, `Artifact`, `ArtifactLease`, `ConnectorOptions`, and `connect(lease:options:)`. The returned `Session` exposes only `RPCPeer`, `ByteStream`, `IncomingStream`, and bounded stream metadata. `ConnectError`, `SessionError`, and `RPCError` are the public failure boundary. `classifyConnectError(_:)` and `classifySessionError(_:)` return an `ErrorRetryClassification` whose action is a `RetryAction`. Concrete carrier sessions and runtime capability descriptors are internal.

## Rust

The `flowersec` crate exposes `Artifact`, `ArtifactError`, `ArtifactLease`, `ArtifactSpendError`, `ConnectorOptions`, `connect(...)`, `ConnectError`, `ConnectErrorCode`, `Session`, `SessionError`, `RpcPeer`, `RpcPeerExt`, `RpcError`, `RpcCallError`, `ByteStream`, `IncomingStream`, `JsonObject`, `StreamTerminalError`, and the carrier-neutral optional `UnreliableMessageChannel`. Rust `SessionError` includes the portable session states `GoingAway`, `StreamRejected`, `StreamReset`, `RekeyFailed`, and `LivenessFailed` while retaining older redacted variants for compatibility. `UnreliableSendOutcome` contains `Accepted`, `DroppedExpired`, `DroppedBudget`, and `DroppedCarrier`, matching the public send-result semantics of Go and TypeScript. `ConnectError::as_str()`, `ConnectErrorCode::as_str()`, `SessionError::as_str()`, and `AcceptErrorCode::as_str()` return canonical public code strings for redacted error text. `classify_connect_error(...)` and `classify_session_error(...)` return an `ErrorRetryClassification` with an `ErrorRetryAction`; `ErrorRetryAction::as_str()` returns its canonical cross-language action name. `RpcPeerExt::call_typed(...)` adds typed JSON encoding and decoding while preserving `RpcCallError::Application`. Native server runtimes additionally use `AcceptorOptions`, `Acceptor`, `AcceptError`, and `AcceptErrorCode` to turn an opaque direct-session artifact into the same carrier-neutral `Session`. Quinn connections, admission frames, capability descriptors, candidate plans, session ledgers, and implementation modules remain crate-private.
