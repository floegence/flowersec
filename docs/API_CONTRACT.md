# Flowersec 4 Public API Contract
...
The source tree defines the Flowersec 4 SDK contract over Transport v3. The
...
The only supported application import is `github.com/floegence/flowersec/flowersec-go/v4`, conventionally named `flowersec`. Its public API is unversioned and every connection uses Transport v3.
...
The Go-only server import `github.com/floegence/flowersec/flowersec-go/v4/controlplane`, conventionally named `controlplane`, issues v3 artifacts and answers the `flowersec-runtime` authorization callback without exposing carrier, candidate, FSB3, PSK, or session-contract objects.
...
current public surface and bind only Transport v3.
