...
Required performance is Go-owned. It measures the production Go runtime's connection and stream control-plane capacity, connect and liveness latency, cleanup, and resource growth; it is not multi-language performance parity and does not represent application business throughput. Six required capacity IDs cover direct WSS/raw QUIC and the WW/QQ/WQ/QW native tunnel topologies. They freeze 1,000 concurrent sessions and fixed RSS, CPU, file-descriptor, goroutine, and task ceilings. Without `FLOWERSEC_PERFORMANCE_BUDGET`, the capacity contract uses a 30-second ramp, 60-second hold, 30-second cleanup, and 120-second watchdog. The integrated runner always supplies a validated `5m..24h` budget, `10m` by default, and sets the native ramp, hold, and cleanup to `round(10s * min(budget / 10m, 5), 10ms)`, `round(7s * min(budget / 10m, 5), 10ms)`, and `round(3s * min(budget / 10m, 5), 10ms)`; the watchdog is their sum, so the standard run uses 10 seconds, 7 seconds, 3 seconds, and 20 seconds. Each case counts attempted, succeeded, and failed sessions; proves a unique active peak of exactly 1,000 with no hold disconnect; records ramp/hold/cleanup resource samples; and finishes with zero watchdogs and zero residual sessions.

`performance/single-connection/{wss,raw-quic}` and `performance/throughput/{wss,raw-quic}` separately measure production encrypted stream payload transfer in the client-to-server, server-to-client, and full-duplex directions. Single-connection cases use one 1 MiB stream; streaming cases use four concurrent streams for each 1 KiB, 64 KiB, and 1 MiB payload coordinate. Every coordinate has three fixed measured windows. Without `FLOWERSEC_PERFORMANCE_BUDGET`, each window is five seconds. The integrated runner always supplies a validated `5m..24h` budget, `10m` by default, and sets the window to `round(700ms * min(budget / 10m, 5), 10ms)`, so the standard run uses 700 ms. Only fully read and content-verified bytes are counted; FIN is required and reset is failure-only. Static minimum throughput and per-sample p95 budgets decide every sample; the case report also records and gates aggregate p50, p95, and p99 acknowledgement latency against the same maximum. No historical baseline, confidence interval, or generated performance evidence is used.

Optional Go WebTransport single-connection, streaming-throughput, capacity, and soak cases, plus Chromium WebTransport capacity cases, form the `performance-optional` registry partition. The integrated `make performance` plan appends that partition after all required cases and runs its WebTransport/Chromium capability preflight first. A successful structured capability result executes the optional cases in the same budget and report. Only a structured result stating that the WebTransport constructor is unavailable records the capability and remaining optional cases as `UNSUPPORTED`; Chromium path, launch, secure-origin navigation, runner, and cleanup failures remain `FAIL` and block the suite. The three browser stream-capacity cases prove 100 production sessions with 128 simultaneously live bidirectional streams per session. The controller distributes those independent sessions evenly across two isolated Chromium processes with one renderer each, while retaining one aggregate resource scope; their headless capacity pages disable GPU and software rasterization because the workload has no visual surface. Without a configured budget they use a 60-second ramp. With a validated `5m..24h` budget, their ramp and cleanup are each at least 20 seconds while their hold retains the shared scaled capacity window; the standard 10-minute run uses a 20-second ramp, 7-second hold, 20-second cleanup, and 47-second watchdog. The aggregate memory ceiling remains 3 GiB. A task-private cgroup supplies the kernel memory peak on newer kernels; kernels without native `memory.peak` use a 10 ms sampled peak of `memory.current`, and hosts where a private cgroup is unavailable fail closed against the sampled process-tree RSS fallback. The cases also use a dedicated 32,768 descriptor ceiling plus a 240 CPU-second aggregate ceiling. Those ceilings preserve measured headroom over the Chromium calibration; the 1,000-session browser cases remain at 12,288 descriptors and required non-browser capacity cases remain at 8,192.
