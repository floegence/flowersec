...
Optional Go WebTransport single-connection, streaming-throughput, capacity, and soak cases, plus Chromium WebTransport capacity cases, form the `performance-optional` registry partition. The integrated `make performance` plan appends that partition after all required cases and runs its WebTransport/Chromium capability preflight first. A successful structured capability result executes the optional cases in the same budget and report. Only a structured result stating that the WebTransport constructor is unavailable records the capability and remaining optional cases as `UNSUPPORTED`; Chromium path, launch, secure-origin navigation, runner, and cleanup failures remain `FAIL` and block the suite. The three browser stream-capacity cases prove 100 production sessions with 128 simultaneously live bidirectional streams per session. Without a configured budget they use a 60-second ramp. With a validated `5m..24h` budget, their ramp is at least 30 seconds, their cleanup is at least 20 seconds, and their hold retains the shared scaled capacity window; the standard 10-minute run uses a 30-second ramp, 7-second hold, 20-second cleanup, and 57-second watchdog. They also use a dedicated 32,768 aggregate process-tree descriptor ceiling plus a 240 CPU-second aggregate ceiling. Those ceilings preserve measured headroom over the Chromium calibration; the 1,000-session browser cases remain at 12,288 descriptors and required non-browser capacity cases remain at 8,192.
