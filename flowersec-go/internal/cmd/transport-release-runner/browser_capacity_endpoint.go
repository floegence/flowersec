...
type browserCapacityProducerResourceSample struct {
	...
	Runner     transportrelease.ResourceSnapshot `json:"runner"`
	Aggregate  transportrelease.ResourceSnapshot `json:"aggregate"`
}
...
type directBrowserCapacityArtifact struct {
	*transportrelease.ProductDirectBrowserArtifact
}

func (*directBrowserCapacityArtifact) Start() {}
...
func openProductionBrowserCapacityEndpoint(ctx context.Context, config browserCapacityEndpointConfig) (*browserCapacityEndpoint, error) {
	...
	if (!heldSessions && !streamCapacity) ||
		...
		net.ParseIP(config.ServerAddress) == nil || !filepath.IsAbs(config.SourceRoot) || !filepath.IsAbs(config.EvidenceDirectory) ||
	...
	entries, err := os.ReadDir(config.EvidenceDirectory)
	if err != nil || len(entries) != 0 {
		return nil, errors.New("browser capacity evidence directory must exist and be empty")
	}
	if ctx == nil {
	...
	if err := linuxnetlab.InNamespace(config.ServerNamespace, func() error {
		if config.Topology == browserDirectWebTransportTopology {
			owner, openErr := transportrelease.OpenProductDirectBrowserStreamCapacityEndpointAt(context.Background(), config.ServerAddress, allowedOrigin)
			if openErr != nil {
				return openErr
			}
	...
	control, wait, evidence, err := startBrowserCapacityControl(ctx, config, eventSinkURL, certificateHash)
	if err != nil {
		broker.cancelAll()
	...
	return &browserCapacityEndpoint{
		...
		evidence: evidence, topology: config.Topology, profileID: config.ProfileID, sessions: config.Sessions, streamsPerSession: config.StreamsPerSession,
		contract: contract, resourcePreflight: resourcePreflight,
		resourceEvidence: filepath.Join(config.EvidenceDirectory, "producer-resource.json"), heldStreams: make(map[string][]flowersession.ByteStream),
...
func (endpoint *browserCapacityEndpoint) Close(ctx context.Context) error {
	...
	endpoint.closeOnce.Do(func() {
		...
		endpoint.closeErr = errors.Join(endpoint.Quiesce(ctx), endpoint.control.Shutdown(ctx), endpoint.wait(ctx), endpoint.writeProducerResourceEvidence())
		endpoint.closeErr = errors.Join(endpoint.closeErr, endpoint.verifyEvidence())
	})
	select {
...
func (endpoint *browserCapacityEndpoint) verifyEvidence() error {
	if len(endpoint.evidence) == 0 {
		return nil
	}
	if len(endpoint.evidence) != 5 {
		return errors.New("browser capacity endpoint has an incomplete evidence inventory")
	}
	for _, path := range endpoint.evidence {
		if info, err := os.Stat(path); err != nil || !info.Mode().IsRegular() || info.Size() == 0 {
			return fmt.Errorf("browser capacity evidence is missing: %s", filepath.Base(path))
		}
	}
	resultData, err := os.ReadFile(endpoint.evidence[2])
	if err != nil {
		return err
	}
	var result struct {
	...
	if result.SchemaVersion != 1 || result.Classification != "raw_chromium_webtransport_capacity" ||
		...
		return errors.New("Chromium capacity evidence does not prove the exact live-session contract")
	}
	...
	if result.CompletedStreams != wantStreams || result.PeakActiveStreams != wantStreams || result.ClosedStreams != wantStreams || result.ResidualStreams != 0 {
		return errors.New("Chromium capacity evidence does not prove the exact stream contract")
	}
	configData, err := os.ReadFile(endpoint.evidence[3])
	if err != nil {
		return err
	}
	var config struct {
	...
	if err := json.Unmarshal(configData, &config); err != nil || config.SchemaVersion != 1 || config.Topology != endpoint.topology ||
		...
		return errors.New("Chromium capacity evidence configuration is mismatched")
	}
	traceHeader := make([]byte, 4)
	trace, err := os.Open(endpoint.evidence[1])
	if err != nil {
		return err
	}
...
func (endpoint *browserCapacityEndpoint) writeProducerResourceEvidence() error {
	if endpoint.resourceEvidence == "" {
		return nil
	}
	...
	data = append(data, '\n')
	return os.WriteFile(endpoint.resourceEvidence, data, 0o600)
}

func (endpoint *browserCapacityEndpoint) EvidencePaths() []string {
	if endpoint == nil {
		return nil
	}
	return append([]string(nil), endpoint.evidence...)
}

func (endpoint *browserCapacityEndpoint) CaptureResourceSnapshot() (transportrelease.ResourceSnapshot, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	controller, err := endpoint.control.Snapshot(ctx)
	if err != nil {
		return transportrelease.ResourceSnapshot{}, fmt.Errorf("capture Chromium capacity resources: %w", err)
	}
	runner, err := transportrelease.CaptureResourceSnapshot()
	if err != nil {
	...
	if snapshot.RSSBytes > contract.MaxRSS || snapshot.CPUNanoseconds > uint64(contract.MaxCPU) ||
		...
		return transportrelease.ResourceSnapshot{}, fmt.Errorf(
	...
	if len(endpoint.resourceSamples) >= len(phases) {
		endpoint.resourceMu.Unlock()
		return transportrelease.ResourceSnapshot{}, errors.New("browser capacity captured too many producer resource samples")
	}
...
func aggregateBrowserCapacityResources(runner transportrelease.ResourceSnapshot, tree linuxProcessTreeSnapshot) (transportrelease.ResourceSnapshot, error) {
	...
	if runner.At.IsZero() || tree.At.IsZero() || tree.RootPID <= 0 || tree.PGID <= 0 || tree.ProcessCount <= 0 || tree.Tasks < tree.ProcessCount ||
		...
		return transportrelease.ResourceSnapshot{}, errors.New("browser capacity aggregate resource counters are invalid")
	}
	at := runner.At
	if tree.At.After(at) {
		at = tree.At
...
func startBrowserCapacityControl(ctx context.Context, config browserCapacityEndpointConfig, eventSinkURL, certificateHash string) (browserCapacityControl, func(context.Context) error, []string, error) {
	...
	plan := map[string]any{
		...
		"evidence_directory": config.EvidenceDirectory, "operation_deadline_ms": config.OperationDeadline.Milliseconds(),
	}
	...
	wait := func(waitCtx context.Context) error {
		...
		case waitErr := <-waitDone:
			...
			stderrErr := writeBrowserCapacityStderr(config.EvidenceDirectory, stderr.Bytes())
	...
	evidence := []string{
		filepath.Join(config.EvidenceDirectory, "chromium-netlog.json"),
		filepath.Join(config.EvidenceDirectory, "chromium-trace.zip"),
		filepath.Join(config.EvidenceDirectory, "controller-result.json"),
		filepath.Join(config.EvidenceDirectory, "controller-config.json"),
		filepath.Join(config.EvidenceDirectory, "producer-resource.json"),
	}
	return control, wait, evidence, nil
}
