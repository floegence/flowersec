...
func (control *fakeBrowserCapacityControl) Connect(_ context.Context, record *browserCapacityRecord) error {
	control.mu.Lock()
	control.connects++
	control.mu.Unlock()
...
func TestAggregateBrowserCapacityResourcesIncludesChromiumProcessTree(t *testing.T) {
	runner := transportrelease.ResourceSnapshot{At: time.Now(), RSSBytes: 100, CPUNanoseconds: 200, AllocatedBytes: 300, OpenFDs: 4, Goroutines: 5, Tasks: 6}
...
func (artifact *fakeBrowserCapacityArtifact) Start() {
	artifact.mu.Lock()
	artifact.starts++
	artifact.mu.Unlock()
}
