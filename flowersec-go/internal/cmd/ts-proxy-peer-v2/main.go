...
func main() {
	...
	proxy, err := flowersec.NewProxyServer(flowersec.ProxyServerOptions{
		...
		OnError: func(err error) {
			fmt.Fprintf(os.Stderr, "v2 proxy handler error: %v\n", err)
		},
	})
	fail(err)
	defer proxy.Close()
	...
	acceptor, err := flowersec.NewAcceptor(flowersec.AcceptorOptions{
		AllowedOrigins: []string{*origin}, MaxInboundStreams: 8,
		Authorize: func(_ context.Context, request controlplane.RuntimeAuthorizationRequest) (controlplane.AuthorizationResponse, error) {
			return controlplane.AuthorizeRuntime(request, record, "proxy-matrix-go-v2")
		},
	...
	issued, err := controlplane.NewIssuer().IssueDirect(controlplane.DirectIssueOptions{
		Session: controlplane.SessionOptions{
			ChannelID: "browser-proxy-go-v2", ExpiresAt: time.Now().Add(time.Minute), MaxInboundStreams: 8,
		},
		Endpoints: endpoints, RendezvousGroupID: "browser-proxy-go-v2",
		ListenerAudience: "browser-proxy-matrix-v2", UpstreamAddress: server.Listener.Addr().String(),
	})
	fail(err)
	...
	case <-time.After(20 * time.Second):
		fail(errors.New("v2 proxy matrix session did not release"))
	}
}
