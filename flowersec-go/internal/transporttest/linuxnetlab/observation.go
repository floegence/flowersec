...
type KernelFaultObservation struct {
...
func validateKernelFaultStats(direction string, stats KernelFaultStats) error {
	...
	if stats.Packets > 0 && (stats.FirstPacketNS == 0 || stats.LastPacketNS < stats.FirstPacketNS) {
		return fmt.Errorf("%s kernel packet timing is invalid: first=%d last=%d", direction, stats.FirstPacketNS, stats.LastPacketNS)
	}
