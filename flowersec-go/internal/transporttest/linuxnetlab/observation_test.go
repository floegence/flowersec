...
func TestValidateKernelFaultObservationEnforcesConservation(t *testing.T) {
	stats := KernelFaultStats{
		...
		FirstPacketNS: 1, LastPacketNS: 2,
	}
...
func TestValidateKernelFaultStatsRejectsDuplicateFailures(t *testing.T) {
	stats := KernelFaultStats{Packets: 1, DeliveredPackets: 1, DuplicateErrors: 1, FirstPacketNS: 1, LastPacketNS: 1}
	if err := validateKernelFaultStats("server", stats); err == nil {
		t.Fatal("accepted failed kernel duplication")
	}
}

func TestValidateKernelFaultStatsRejectsInvalidPacketTiming(t *testing.T) {
	stats := KernelFaultStats{Packets: 1, DeliveredPackets: 1, FirstPacketNS: 2, LastPacketNS: 1}
	if err := validateKernelFaultStats("client", stats); err == nil {
		t.Fatal("accepted kernel packet timing that moved backwards")
	}
}
