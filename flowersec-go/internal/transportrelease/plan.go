...
func LoadReleasePlan(path string) (ReleasePlan, ManifestBinding, error) {
	...
	for _, profile := range manifest.Profiles {
		...
		if profile.Mode != "forced" || profile.Cold == nil || profile.RPC == nil || profile.Bulk == nil {
			return ReleasePlan{}, ManifestBinding{}, errors.New("clean-v1 workload is incomplete")
		}
		plan := ReleasePlan{RunCount: manifest.RunCount, Clean: ProfilePlan{
		...
		if err := validateProfilePlan(plan.Clean); err != nil {
			return ReleasePlan{}, ManifestBinding{}, err
		}
		return plan, binding, nil
	}
	return ReleasePlan{}, ManifestBinding{}, errors.New("performance manifest has no clean-v1 profile")
}
