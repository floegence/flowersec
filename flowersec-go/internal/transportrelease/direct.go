// Package transportrelease contains the production workloads used by the
// privileged Transport v2 release-evidence runner.
...
func openDirectWebSocket(ctx context.Context, serverTLS, clientTLS *tls.Config) (carrier.Session, carrier.Session, []func() error, error) {
	...
	go func() {
		session, sessionErr := carrierws.NewAfterAdmission(serverConn, carrierws.ServerRole, carrierws.SubprotocolDirect, resources, carrierws.LivenessPolicy{})
		serverResult <- struct {
			session *carrierws.Session
			err     error
		}{session: session, err: sessionErr}
	}()
	client, err := carrierws.NewAfterAdmission(clientConn, carrierws.ClientRole, carrierws.SubprotocolDirect, resources, carrierws.LivenessPolicy{})
	if err != nil {
		_ = serverConn.Close()
		return nil, nil, closers, err
	}
