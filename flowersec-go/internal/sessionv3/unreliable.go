...
func (*unreliableChannel) MaxMessageBytes() int { return protocolv3.MaxUnreliableMessageBytes }
...
func (channel *unreliableChannel) Send(ctx context.Context, payload []byte, options UnreliableSendOptions) (UnreliableSendStatus, error) {
	...
	if len(payload) == 0 || len(payload) > protocolv3.MaxUnreliableMessageBytes {
	...
	header := protocolv3.UnreliableHeader{
		...
		CiphertextLength: uint32(len(payload) + protocolv3.AEADTagBytes),
	}
	material, err := protocolv3.DeriveUnreliableMaterial(roots.EpochSecret, channel.session.h3, channel.session.sendDir, epoch)
	if err != nil {
		return "", err
	}
	ciphertext, err := protocolv3.SealUnreliable(channel.session.config.Suite, material, channel.session.h3, channel.session.sendDir, header, payload)
	if err != nil {
		return "", err
	}
...
func (channel *unreliableChannel) open(wire []byte, now time.Time) ([]byte, bool) {
	if len(wire) < protocolv3.UnreliableHeaderSize || len(wire) > protocolv3.MaxUnreliableWireBytes {
		return nil, false
	}
	header, err := protocolv3.ParseUnreliableHeader(wire[:protocolv3.UnreliableHeaderSize])
	if err != nil || len(wire) != protocolv3.UnreliableHeaderSize+int(header.CiphertextLength) ||
	...
	material, err := protocolv3.DeriveUnreliableMaterial(roots.EpochSecret, channel.session.h3, channel.session.recvDir, header.Epoch)
	if err != nil {
		return nil, false
	}
	plaintext, err := protocolv3.OpenUnreliable(channel.session.config.Suite, material, channel.session.h3, channel.session.recvDir, header, wire[protocolv3.UnreliableHeaderSize:])
...
func (channel *unreliableChannel) takeSequence() (uint64, error) {
	...
	if channel.sendExhausted {
		return 0, protocolv3.ErrCounterExhausted
	}
...
func (s *engineSession) unreliableSendRoots() (uint32, protocolv3.EpochRoots, error) {
	s.cryptoMu.RLock()
	defer s.cryptoMu.RUnlock()
	roots, ok := s.sendRoots[s.sendEpoch]
	if !ok {
		return 0, protocolv3.EpochRoots{}, ErrSessionProtocol
	}
	return s.sendEpoch, roots, nil
}

func (s *engineSession) unreliableReceiveRoots(epoch uint32) (protocolv3.EpochRoots, bool) {
	s.cryptoMu.RLock()
	defer s.cryptoMu.RUnlock()
	if epoch > s.recvSessionEpoch {
		return protocolv3.EpochRoots{}, false
	}
