...
func (s *engineSession) Close() error {
	s.closeOnce.Do(func() {
		...
		protocolErr = errors.Join(protocolErr, s.sendControl(protocolv3.InnerSessionClose, []byte{0, 1}))
...
func (s *engineSession) handlePeerSessionClose() {
	...
	s.closeOnce.Do(func() {
		...
		closeContext, cancelClose := context.WithTimeout(context.Background(), sessionCloseFlushTimeout)
		defer cancelClose()
		protocolErr := s.sendControl(protocolv3.InnerSessionClose, []byte{0, 1})
		protocolErr = errors.Join(protocolErr, s.flushControl(closeContext))
		protocolErr = errors.Join(protocolErr, s.control.CloseWrite())
...
func (s *engineSession) sendGoAway(reason uint16) error {
	...
	s.openMu.Unlock()
	return s.sendControl(protocolv3.InnerGoAway, marshalIDReason(lastAccepted, reason))
}
