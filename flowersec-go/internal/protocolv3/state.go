...
func (l *StreamLedger) PeerReset(id uint64) error {
	...
	switch l.State(id) {
	case LedgerUnseen:
		l.setState(id, LedgerAbandonedNoFSS3)
	case LedgerAbandonedNoFSS3, LedgerUsedOrTerminal:
...
func (l *StreamLedger) ValidFSS3(id uint64) error {
	if err := l.validateID(id); err != nil {
		return err
	}
...
func (l *StreamLedger) ValidFSS3ForAbandoned(id uint64) (LateSetupAction, error) {
	if err := l.validateID(id); err != nil {
		return 0, err
	}
	state := l.State(id)
	if state != LedgerAbandonedNoFSS3 {
...
func (l *StreamLedger) advanceFrontier() {
	...
	for {
		...
		if state != LedgerAbandonedNoFSS3 && state != LedgerUsedOrTerminal {
			return
		}
		l.frontier = next
