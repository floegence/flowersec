// Package protocolv3 implements Flowersec v3's fixed binary wire envelopes.
package protocolv3
...
var (
	ErrInvalidSetupPreface = errors.New("invalid FSS3 setup preface")
	ErrInvalidRecordHeader = errors.New("invalid FSR3 record header")
	ErrRecordTooLarge      = errors.New("FSR3 record too large")
	ErrInvalidInnerRecord  = errors.New("invalid FSR3 inner record")
	ErrUnknownInnerType    = errors.New("unknown FSR3 inner type")
)
...
func ParseSetupPreface(raw []byte) (SetupPreface, error) {
	if len(raw) != SetupPrefaceSize || string(raw[0:4]) != "FSS3" || raw[4] != 3 ||
...
func ParseRecordHeader(raw []byte) (RecordHeader, error) {
	if len(raw) != RecordHeaderSize || string(raw[0:4]) != "FSR3" || raw[4] != 3 ||
