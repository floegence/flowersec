...
var (
	ErrInvalidOpenPayload  = errors.New("invalid Flowersec v3 OPEN payload")
	ErrInvalidOpenKind     = errors.New("invalid Flowersec v3 OPEN kind")
	ErrInvalidOpenMetadata = errors.New("invalid Flowersec v3 OPEN metadata")
	ErrInvalidOpenACK      = errors.New("invalid Flowersec v3 OPEN_ACK payload")
	ErrInvalidOpenReject   = errors.New("invalid Flowersec v3 OPEN_REJECT payload")
)
...
func ComputeFSS3Hash(rawFSS3 []byte) ([32]byte, error) {
	if _, err := ParseSetupPreface(rawFSS3); err != nil {
		return [32]byte{}, err
	}
	return sha256.Sum256(rawFSS3), nil
}
...
func ComputeOpenHash(rawOpenPayload []byte) ([32]byte, error) {
	...
	preimage := make([]byte, 0, len("flowersec-v3-open\x00")+4+len(rawOpenPayload))
	preimage = append(preimage, "flowersec-v3-open\x00"...)
	var size [4]byte
