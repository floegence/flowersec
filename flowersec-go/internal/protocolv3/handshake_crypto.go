...
var (
	ErrInvalidEphemeralKey = errors.New("invalid Flowersec v3 ephemeral key")
	ErrInvalidSharedSecret = errors.New("invalid Flowersec v3 ECDH shared secret")
	ErrInvalidHandshakePSK = errors.New("invalid Flowersec v3 handshake PSK")
	ErrInvalidTranscript   = errors.New("invalid Flowersec v3 handshake transcript")
)
...
func ComputeServerConfirm(handshakePRK, h1 [32]byte) ([32]byte, [32]byte, error) {
	info := append([]byte("flowersec v3 server finished"), h1[:]...)
...
func ComputeClientConfirm(handshakePRK, h2 [32]byte) ([32]byte, [32]byte, error) {
	info := append([]byte("flowersec v3 client finished"), h2[:]...)
	keyBytes, err := internalhkdf.ExpandSHA256(handshakePRK, info, 32)
	if err != nil {
		return [32]byte{}, [32]byte{}, err
	}
