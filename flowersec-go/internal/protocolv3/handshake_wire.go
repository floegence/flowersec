...
var (
	ErrInvalidControlPreface    = errors.New("invalid FSC3 control preface")
	ErrInvalidHandshakeFrame    = errors.New("invalid FSH3 handshake frame")
	ErrHandshakePayloadTooLarge = errors.New("FSH3 handshake payload too large")
	ErrInvalidHandshakeMessage  = errors.New("invalid Flowersec v3 handshake message")
	ErrHandshakeBinding         = errors.New("Flowersec v3 handshake binding mismatch")
)
...
func ParseControlPreface(raw []byte) error {
	if len(raw) != ControlPrefaceSize || string(raw[0:4]) != "FSC3" || raw[4] != 3 || raw[5] != byte(RoleClient) {
...
func ReadHandshakeFrame(reader io.Reader) (*HandshakeFrame, error) {
	...
	if string(header[0:4]) != "FSH3" || header[4] != 3 || header[6] != 0 || header[7] != 0 {
...
func validateClientInitFields(message ClientInit) error {
	if message.Profile != "flowersec/3" || !validHandshakeRegistryID(message.ChannelID, false) || message.ClientRole != byte(RoleClient) ||
