...
var (
	ErrInvalidUnreliableHeader = errors.New("invalid FSD3 unreliable message header")
	ErrUnreliableTooLarge      = errors.New("FSD3 unreliable message too large")
)
...
func ParseUnreliableHeader(raw []byte) (UnreliableHeader, error) {
	if len(raw) != UnreliableHeaderSize || string(raw[0:4]) != "FSD3" || raw[4] != 3 || raw[5] != 0 ||
...
func DeriveUnreliableMaterial(epochSecret, h3 [32]byte, direction Direction, epoch uint32) (UnreliableMaterial, error) {
	...
	root, err := expand32(epochSecret, labelWith("flowersec v3 unreliable root"))
	if err != nil {
		return UnreliableMaterial{}, err
	}
	var epochBytes [4]byte
	binary.BigEndian.PutUint32(epochBytes[:], epoch)
	secret, err := expand32(root, labelWith("flowersec v3 unreliable", h3[:], []byte{byte(direction)}, epochBytes[:]))
	if err != nil {
		return UnreliableMaterial{}, err
	}
	key, err := expand32(secret, labelWith("flowersec v3 unreliable key"))
	if err != nil {
		return UnreliableMaterial{}, err
	}
...
func sealOpenUnreliable(seal bool, suite Suite, material UnreliableMaterial, h3 [32]byte, direction Direction, header UnreliableHeader, payload []byte) ([]byte, error) {
	...
	aad := labelWith("flowersec-v3-unreliable", h3[:], []byte{byte(direction)}, rawHeader)
	if seal {
