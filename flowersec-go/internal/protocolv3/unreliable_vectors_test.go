...
func TestSharedUnreliableVectors(t *testing.T) {
	...
	raw, err := os.ReadFile(filepath.Join(filepath.Dir(sourceFile), "../../../testdata/transport_v3/datagram_vectors.json"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := decoder.Decode(&fixtures); err != nil || fixtures.SchemaVersion != 3 || len(fixtures.Vectors) == 0 {
...
func verifyUnreliableVector(t *testing.T, vector unreliableVector) {
	...
	secret, err := DeriveNextEpoch(roots.RekeyRoot, h3, vector.Direction, vector.Epoch)
	if err != nil {
		t.Fatal(err)
	}
	...
	aad := labelWith("flowersec-v3-unreliable", h3[:], []byte{byte(vector.Direction)}, rawHeader)
