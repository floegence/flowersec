...
var (
	ErrInvalidDirection = errors.New("invalid v3 direction")
	ErrInvalidSuite     = errors.New("invalid v3 suite")
	ErrAuthentication   = errors.New("v3 record authentication failed")
)
...
func DeriveEpochZero(sessionPRK [32]byte, direction Direction) (EpochRoots, error) {
	...
	epochSecret, err := expand32(sessionPRK, labelWith("flowersec v3 epoch zero", []byte{byte(direction)}))
	if err != nil {
		return EpochRoots{}, err
	}
...
func DeriveNextEpoch(rekeyRoot, h3 [32]byte, direction Direction, nextEpoch uint32) ([32]byte, error) {
	...
	return expand32(rekeyRoot, labelWith("flowersec v3 next epoch", h3[:], []byte{byte(direction)}, epoch[:]))
}
...
func deriveRoots(epochSecret [32]byte) (EpochRoots, error) {
	control, err := expand32(epochSecret, labelWith("flowersec v3 control root"))
	if err != nil {
		return EpochRoots{}, err
	}
	stream, err := expand32(epochSecret, labelWith("flowersec v3 stream root"))
	if err != nil {
		return EpochRoots{}, err
	}
	setup, err := expand32(epochSecret, labelWith("flowersec v3 setup root"))
	if err != nil {
		return EpochRoots{}, err
	}
	rekey, err := expand32(epochSecret, labelWith("flowersec v3 rekey root"))
	if err != nil {
		return EpochRoots{}, err
	}
	return EpochRoots{
...
func DeriveStreamMaterial(streamRoot, h3 [32]byte, logicalStreamID uint64, direction Direction, epoch uint32) (RecordMaterial, error) {
	...
	return deriveRecordMaterial(streamRoot, "flowersec v3 stream", h3, logicalStreamID, direction, epoch)
}

func DeriveControlMaterial(controlRoot, h3 [32]byte, direction Direction, epoch uint32) (RecordMaterial, error) {
	return deriveRecordMaterial(controlRoot, "flowersec v3 control", h3, 0, direction, epoch)
}
...
func deriveRecordMaterial(root [32]byte, secretLabel string, h3 [32]byte, logicalStreamID uint64, direction Direction, epoch uint32) (RecordMaterial, error) {
	...
	key, err := expand32(secret, labelWith("flowersec v3 record key"))
	if err != nil {
		return RecordMaterial{}, err
	}
	nonce, err := internalhkdf.ExpandSHA256(secret, labelWith("flowersec v3 nonce"), 4)
	if err != nil {
		return RecordMaterial{}, err
	}
	var prefix [4]byte
...
func ComputeSetupMAC(setupRoot, h3 [32]byte, preface SetupPreface) ([32]byte, error) {
	...
	_, _ = mac.Write(labelWith("flowersec-v3-setup"))
	_, _ = mac.Write(h3[:])
...
func recordAAD(h3 [32]byte, logicalStreamID uint64, direction Direction, rawHeader []byte) []byte {
	var id [8]byte
	binary.BigEndian.PutUint64(id[:], logicalStreamID)
	out := labelWith("flowersec-v3-record", h3[:], id[:], []byte{byte(direction)}, rawHeader)
	return out
}
...
func unreliableNoncePrefix(secret [32]byte) ([4]byte, error) {
	raw, err := internalhkdf.ExpandSHA256(secret, labelWith("flowersec v3 unreliable nonce"), 4)
	if err != nil {
		return [4]byte{}, err
	}
	var out [4]byte
