...
func TestOpenUnicodeAndCanonicalMetadataVectors(t *testing.T) {
	...
	for _, vector := range fixture.Positive {
		t.Run(vector.ID, func(t *testing.T) {
			raw, err := protocolv3.MarshalOpenPayload(protocolv3.OpenPayload{
				LogicalStreamID: 1,
				Kind:            vector.Kind,
			...
			decoded, err := protocolv3.ParseOpenPayload(raw)
			if err != nil {
...
func TestOpenUnicodeUsesReviewedGo15TablePlus15_1Delta(t *testing.T) {
	...
	for _, input := range []protocolv3.OpenPayload{
		...
		if _, err := protocolv3.MarshalOpenPayload(input); err != nil {
...
func loadOpenUnicodeFixture(t *testing.T) openUnicodeFixture {
	t.Helper()
	raw, err := os.ReadFile("../../../testdata/transport_v3/open_unicode_vectors.json")
	if err != nil {
		t.Fatal(err)
	}
