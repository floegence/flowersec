...
// reader/publisher. The shared SendService remains the only record sender.
// Reader progress never waits for observer decoding or callbacks.
type NotifyChannel struct {
	mu                                          sync.Mutex
	owner                                       *StreamOwnership
	writer                                      *RPCBatchWriter
	publisher                                   *rpcv4.NotifyPublisher
...
func NotifyChannelCharge(runtimeBytes uint64) (resourcev4.Vector, error) {
	if runtimeBytes == 0 {
		return resourcev4.Vector{}, cryptov4.ErrConfiguration
	}
	return (resourcev4.Vector{resourcev4.SDKBytes: uint64(unsafe.Sizeof(NotifyChannel{})), resourcev4.Items: 1, resourcev4.Tasks: 2, resourcev4.WorkSlots: 2, resourcev4.Timers: 1}).Add(resourcev4.Vector{resourcev4.SDKBytes: runtimeBytes})
}
...
func NewNotifyChannel(owner *StreamOwnership, routes *rpcv4.ContractRoutes, channel [16]byte, receiverConfig rpcv4.NotifyReceiverConfig, publisherConfig rpcv4.NotifyPublisherConfig, metadata, batch, publish, receive resourcev4.Reference, runtimeBytes uint64) (_ *NotifyChannel, err error) {
	if owner == nil || routes == nil || channel == ([16]byte{}) {
		return nil, cryptov4.ErrConfiguration
	}
	charge, err := NotifyChannelCharge(runtimeBytes)
	if err != nil {
		return nil, err
	}
	...
	c.writer, err = newNotifyBatchWriter(owner, batch, runtimeBytes)
	if err != nil {
		return nil, err
	}
	c.publisher, err = rpcv4.NewNotifyPublisher(c.writer, publisherConfig, publish)
	if err != nil {
		return nil, err
	}
	c.receiver, err = rpcv4.NewNotifyReceiver(routes, receiverConfig, receive)
	if err != nil {
		return nil, err
	}
...
func (c *NotifyChannel) Publisher() *rpcv4.NotifyPublisher {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.publisher
}
func (c *NotifyChannel) Receiver() *rpcv4.NotifyReceiver {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.receiver
}

func (c *NotifyChannel) Association() rpcv4.Association {
	return rpcv4.Association{Channel: c.identity}
}

func (c *NotifyChannel) availablePublisher() *rpcv4.NotifyPublisher {
	if c == nil {
		return nil
	}
	c.mu.Lock()
...
func (c *NotifyChannel) Run(ctx context.Context) (err error) {
	if c == nil || ctx == nil {
	...
	for {
		...
		if result.Progress.Filled > 0 {
			if err := receiver.Feed(c.buffer[:result.Progress.Filled]); err != nil {
				return err
			}
...
func (c *NotifyChannel) publish(ctx context.Context, p *rpcv4.NotifyPublisher, w *RPCBatchWriter) error {
	...
	for {
		...
		if err != nil && !errors.Is(err, cryptov4.ErrCapacity) && !errors.Is(err, rpcv4.ErrCapacity) {
			return err
		}
		if progressed {
			continue
		...
		if active {
			timer.Reset(time.Duration(min(milliseconds, uint64((1<<63-1)/time.Millisecond))) * time.Millisecond)
			deadline = timer.C
		}
		select {
		case <-ctx.Done():
...
func (c *NotifyChannel) WaitCleanup(ctx context.Context) error {
	if c == nil {
		return nil
	}
	if ctx == nil {
