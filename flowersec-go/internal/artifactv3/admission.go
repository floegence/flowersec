...
func MarshalRequest(request Request) ([]byte, error) {
	...
	if len(payload) > MaxCanonicalFSB3Payload {
		return nil, ErrFSB3PayloadTooLarge
	}
	out := make([]byte, FSB3HeaderSize+len(payload))
	copy(out[0:4], "FSB3")
	out[4] = 3
...
func ReadRequest(reader io.Reader) (*DecodedRequest, error) {
	var header [FSB3HeaderSize]byte
	if _, err := io.ReadFull(reader, header[:]); err != nil {
		return nil, fmt.Errorf("%w: header: %v", ErrInvalidFSB3, err)
	}
	if string(header[0:4]) != "FSB3" || header[4] != 3 || header[6] != 0 || header[7] != 0 {
		return nil, ErrInvalidFSB3
	}
	...
	if payloadLength > MaxCanonicalFSB3Payload {
		return nil, ErrFSB3PayloadTooLarge
	}
	if payloadLength == 0 {
		return nil, ErrInvalidFSB3
	...
	if _, err := io.ReadFull(reader, payload); err != nil {
		return nil, fmt.Errorf("%w: payload: %v", ErrInvalidFSB3, err)
	}
	...
	if !bytes.Equal(payload, canonical) {
		return nil, ErrNonCanonicalFSB3
	}
	raw := make([]byte, FSB3HeaderSize+len(payload))
	copy(raw, header[:])
	copy(raw[FSB3HeaderSize:], payload)
...
func ParseRequest(raw []byte) (*DecodedRequest, error) {
	...
	if reader.Len() != 0 {
		return nil, fmt.Errorf("%w: bytes after declared payload", ErrInvalidFSB3)
	}
	return request, nil
}
...
func AcceptorAdmissionsHash(rawFrames [][]byte) ([32]byte, error) {
	if len(rawFrames) < 1 || len(rawFrames) > MaxCandidates {
		return [32]byte{}, ErrInvalidFSB3
	}
	type admission struct {
		candidateID string
		raw         []byte
	}
	admissions := make([]admission, 0, len(rawFrames))
	seen := make(map[string]struct{}, len(rawFrames))
	for _, raw := range rawFrames {
		decoded, err := ParseRequest(raw)
		if err != nil {
			return [32]byte{}, err
		}
		id := decoded.Request.ChosenCandidateID
		if _, duplicate := seen[id]; duplicate {
			return [32]byte{}, ErrInvalidFSB3
		}
		seen[id] = struct{}{}
		admissions = append(admissions, admission{candidateID: id, raw: slices.Clone(raw)})
	}
	slices.SortFunc(admissions, func(left, right admission) int {
...
func validateRequest(request Request) error {
	if request.PathKind != PathDirect && request.PathKind != PathTunnel || request.Profile != Profile {
		return fmt.Errorf("%w: path/profile", ErrInvalidFSB3)
	}
	if !validRegistryID(request.ChannelID, 128) || !validRegistryID(request.RendezvousGroupID, 128) || !validRegistryID(request.ListenerAudience, 128) {
		return fmt.Errorf("%w: common registry id", ErrInvalidFSB3)
	}
	...
	for _, candidate := range request.Candidates {
		source = append(source, Candidate{
			...
			NormalizedURL: candidate.NormalizedURL, TLS: CloneTLSPolicy(candidate.TLS), WireProfile: candidate.WireProfile,
		})
	}
	canonicalCandidates, _, candidateHash, err := CanonicalizeCandidates(request.PathKind, source)
	if err != nil {
		return fmt.Errorf("%w: %v", ErrInvalidFSB3, err)
	}
	if !slices.EqualFunc(canonicalCandidates, request.Candidates, equalCanonicalCandidate) || candidateHash != request.CandidateSetHash {
		return fmt.Errorf("%w: candidate set hash or ordering", ErrInvalidFSB3)
	}
	chosen := false
	for _, candidate := range request.Candidates {
	...
	if !chosen {
		return fmt.Errorf("%w: chosen candidate", ErrInvalidFSB3)
	}
	...
	case PathDirect:
		if !validASCII(request.RoutingToken, MaxAdmissionCredentialBytes) || request.Role != 0 || request.EndpointInstanceID != "" || request.AttachToken != "" {
			return fmt.Errorf("%w: direct variant", ErrInvalidFSB3)
		}
	case PathTunnel:
		if (request.Role != 1 && request.Role != 2) || !validRegistryID(request.EndpointInstanceID, 128) || !validASCII(request.AttachToken, MaxAdmissionCredentialBytes) || request.RoutingToken != "" {
			return fmt.Errorf("%w: tunnel variant", ErrInvalidFSB3)
		}
	}
	return nil
}

func equalCanonicalCandidate(left, right CanonicalCandidate) bool {
	return left.Carrier == right.Carrier && left.ID == right.ID && left.NormalizedURL == right.NormalizedURL &&
		left.WireProfile == right.WireProfile && left.TLS.Mode == right.TLS.Mode && slices.Equal(left.TLS.Pins, right.TLS.Pins)
}
...
func decodeRequestPayload(kind PathKind, payload []byte) (Request, error) {
	if kind == PathDirect {
		...
		if err := decodeStrictJSON(payload, &wire); err != nil {
			return Request{}, fmt.Errorf("%w: direct payload: %v", ErrInvalidFSB3, err)
		}
	...
	if err := decodeStrictJSON(payload, &wire); err != nil {
		return Request{}, fmt.Errorf("%w: tunnel payload: %v", ErrInvalidFSB3, err)
	}
	sessionHash, candidateHash, err := decodeRequestHashes(wire.SessionContractHashBase64URL, wire.CandidateSetHashBase64URL)
	if err != nil {
		return Request{}, err
...
func decodeRequestHashes(sessionValue, candidateValue string) ([32]byte, [32]byte, error) {
	...
	if err != nil {
		return [32]byte{}, [32]byte{}, fmt.Errorf("%w: session contract hash", ErrInvalidFSB3)
	}
	candidateHash, err := decode32(candidateValue)
	if err != nil {
		return [32]byte{}, [32]byte{}, fmt.Errorf("%w: candidate set hash", ErrInvalidFSB3)
	}
...
func MarshalResponse(response AdmissionResponse, reasons ReasonRegistry) ([]byte, error) {
	...
	out := make([]byte, FSA3HeaderSize+len(response.Reason))
	copy(out[0:4], "FSA3")
	out[4] = 3
...
// ReadClientResponse validates a bounded FSA3 response without making
...
func readResponse(reader io.Reader, reasons ReasonRegistry, requireRegisteredReason bool) (AdmissionResponse, error) {
	var header [FSA3HeaderSize]byte
	if _, err := io.ReadFull(reader, header[:]); err != nil {
		return AdmissionResponse{}, fmt.Errorf("%w: header: %v", ErrInvalidFSA3, err)
	}
	if string(header[0:4]) != "FSA3" || header[4] != 3 {
		return AdmissionResponse{}, ErrInvalidFSA3
	}
	...
	if _, err := io.ReadFull(reader, reason); err != nil {
		return AdmissionResponse{}, fmt.Errorf("%w: reason: %v", ErrInvalidFSA3, err)
	}
...
func ParseResponse(raw []byte, reasons ReasonRegistry) (AdmissionResponse, error) {
	...
	if reader.Len() != 0 {
		return AdmissionResponse{}, fmt.Errorf("%w: bytes after declared reason", ErrInvalidFSA3)
	}
	return response, nil
}

// ParseClientResponse validates one exact client-facing FSA3 message.
...
func forbiddenTransportSecurityReason(reason string) bool {
	switch reason {
	case "browser_pin_opaque", "ca_untrusted", "pin_mismatch", "pin_tls_unknown",
