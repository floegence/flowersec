...
func validateSessionFields(session SessionContract) error {
	...
	if session.InitExpireAtUnixSeconds <= 0 || session.InitExpireAtUnixSeconds > maxSignedSafeInteger {
...
func CanonicalizeCandidates(kind PathKind, candidates []Candidate) ([]CanonicalCandidate, []byte, [32]byte, error) {
	...
	for _, candidate := range candidates {
		...
		if err := ValidateTLSPolicy(candidate.TLS); err != nil {
			return nil, nil, [32]byte{}, err
		}
		tuple := string(candidate.Carrier) + "\x00" + string(kind) + "\x00" + normalized
		...
		item := CanonicalCandidate{
			...
			TLS:           CloneTLSPolicy(candidate.TLS),
...
func normalizeCandidateURL(kind PathKind, carrier Carrier, raw string) (string, error) {
	...
	if separator <= 0 || !schemePattern.MatchString(raw[:separator]) {
	...
	case CarrierWebSocket:
		wantScheme = "wss"
		wantPath = "/flowersec/v3/" + string(kind)
	case CarrierRawQUIC:
		wantScheme = "quic"
		if path != "" && path != "/" {
	...
	case CarrierWebTransport:
		wantScheme = "https"
		wantPath = "/flowersec/webtransport/v3/" + string(kind)
	default:
...
func normalizeAuthority(authority string) (string, string, error) {
	...
	if strings.HasPrefix(authority, "[") {
		...
		if strings.Contains(host, ".") {
			return "", "", fmt.Errorf("%w: embedded IPv4", ErrInvalidCandidate)
		}
...
func normalizeDNSOrIPv4(host string) (string, error) {
	...
	if dot := strings.LastIndexByte(canonical, '.'); dot >= 0 {
		last = canonical[dot+1:]
	}
	if decimalLabel.MatchString(last) || hexLabel.MatchString(last) {
		return "", fmt.Errorf("%w: host ends in a number", ErrInvalidCandidate)
	}
