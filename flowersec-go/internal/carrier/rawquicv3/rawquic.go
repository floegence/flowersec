// Package rawquicv3 adapts native QUIC bidirectional streams to Flowersec's
...
const (
	ALPNDirect = "flowersec-direct/3"
	ALPNTunnel = "flowersec-tunnel/3"
...
var (
	...
	ErrSessionResumption       = errors.New("raw QUIC TLS session resumption is forbidden")
)
...
func newSession(conn *quic.Conn, transport *quic.Transport, packetConn net.PacketConn, capacity uint16) (*Session, error) {
	...
	if state.TLS.DidResume {
		return nil, ErrSessionResumption
	}
...
// OpenAdmissionStream creates the client-owned native stream for the FSB3/FSA3
...
func prepareTLS(config *tls.Config, server bool) (*tls.Config, error) {
	...
	if server {
		if config.GetConfigForClient != nil {
			return nil, fmt.Errorf("%w: dynamic server TLS configuration is forbidden", ErrInvalidTLS)
		}
		...
		return nil, fmt.Errorf("%w: client certificate verification is required", ErrInvalidTLS)
	}
	...
	if server {
		clone.SessionTicketsDisabled = true
	}
	return clone, nil
}
