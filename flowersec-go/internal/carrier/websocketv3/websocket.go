// Package websocketv3 adapts a WebSocket connection and hop-local Yamux to
// Flowersec's transport-neutral carrier contract. Every v3 WebSocket requires
// a fresh TLS 1.3 connection.
package websocketv3
...
import (
	...
	fsyamux "github.com/floegence/flowersec/flowersec-go/v4/internal/mux/yamux"
	gorillaws "github.com/gorilla/websocket"
)

const (
	SubprotocolDirect = "flowersec.direct.v3"
	SubprotocolTunnel = "flowersec.tunnel.v3"
...
var (
	...
	ErrInvalidSubprotocol      = errors.New("invalid WebSocket v3 subprotocol")
	ErrTLS13Required           = errors.New("WebSocket carrier requires TLS 1.3")
	ErrTLSResumptionForbidden  = errors.New("WebSocket carrier forbids TLS resumption")
...
// NewAfterAdmission switches an authenticated v3 WebSocket from its bounded
...
// one exact registered v3 subprotocol and a fresh TLS 1.3 connection are
// already negotiated.
...
func ValidateServerRequest(request *http.Request) error {
	...
	return validateTLSState(*request.TLS)
}
...
func validateTransportSecurity(conn *gorillaws.Conn) error {
	underlying := conn.NetConn()
	if tlsConn, ok := underlying.(*tls.Conn); ok {
		return validateTLSState(tlsConn.ConnectionState())
	}
	return ErrTLS13Required
}

func validateTLSState(state tls.ConnectionState) error {
