...
func (s *engineSession) commitControlPriorityLocked(typ protocolv2.InnerType, payload []byte, publish func() error, priority controlPriority, terminal bool) error {
	if s.controlTerminalSealed || (s.controlClosing && !terminal && typ != protocolv2.InnerStreamReset) {
...
func (s *engineSession) closeControlTerminal(ctx context.Context, goAwayPayload []byte) error {
	...
	if goAwayPayload != nil {
		protocolErr = s.commitControlPriorityLocked(protocolv2.InnerGoAway, goAwayPayload, nil, controlPriorityCritical, true)
	}
	protocolErr = errors.Join(protocolErr, s.commitControlPriorityLocked(protocolv2.InnerSessionClose, []byte{0, 1}, nil, controlPriorityCritical, true))
...
func (s *engineSession) handleControl(typ protocolv2.InnerType, payload []byte) error {
	...
	case protocolv2.InnerPing:
		...
		err := s.commitControlPriorityWait(s.ctx, protocolv2.InnerPong, payload, controlPriorityLiveness)
