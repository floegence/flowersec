	...
	for _, preexisting := range []bool{false, true} {
		t.Run(fmt.Sprintf("preexisting=%t", preexisting), func(t *testing.T) {
			...
			session, err := newEngineSession(nil, nil, Config{
				Suite: protocolv2.SuiteChaCha20Poly1305, MaxInboundStreams: 1,
			...
			nextSecret, err := protocolv2.DeriveNextEpoch(currentRoots.RekeyRoot, session.h3, session.recvDir, 1)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTerminalCloseOmitsPriorGoAwayAndPreservesTuple(t *testing.T) {
	...
	session.ledger = protocolv2.NewStreamLedger(protocolv2.RoleServer, protocolv2.MaxStreamLedgerSlots)
