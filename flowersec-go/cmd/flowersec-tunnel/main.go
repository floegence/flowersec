...
func run(args []string, stdout io.Writer, stderr io.Writer) int {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(out, "  flowersec-tunnel [flags]")
		fmt.Fprintln(out, "")
		fmt.Fprintln(out, "Examples:")
		fmt.Fprintln(out, "  # Local dev tunnel (stdout is a single ready JSON object).")
		fmt.Fprintln(out, "  flowersec-tunnel \\")
		fmt.Fprintln(out, "    --listen 127.0.0.1:8080 \\")
		fmt.Fprintln(out, "    --ws-path /ws \\")
		fmt.Fprintln(out, "    --issuer-keys-file ./issuer_keys.json \\")
