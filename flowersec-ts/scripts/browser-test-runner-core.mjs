...
const BROWSER_ENGINES = new Set(["chromium", "firefox"]);
export function chromiumExecutablePath(playwright, environment = process.env) {
  const configured = environment?.FLOWERSEC_CHROMIUM_EXECUTABLE;
  if (configured !== undefined) return absolutePath(configured, "configured Chromium executable");
  if (playwright === null || typeof playwright !== "object" || typeof playwright.executablePath !== "function") {
    throw new TypeError("Playwright Chromium executable resolver is required");
  }
  return absolutePath(playwright.executablePath(), "Playwright Chromium executable");
}

export function firefoxExecutablePath(playwright, environment = process.env) {
  const configured = environment?.FLOWERSEC_FIREFOX_EXECUTABLE;
  if (configured !== undefined) return absolutePath(configured, "configured Firefox executable");
  if (playwright === null || typeof playwright !== "object" || typeof playwright.executablePath !== "function") {
    throw new TypeError("Playwright Firefox executable resolver is required");
  }
  return absolutePath(playwright.executablePath(), "Playwright Firefox executable");
}

export async function verifyChromiumWebTransportCapability(playwright, chromiumExecutable) {
  if (playwright === null || typeof playwright !== "object" || typeof playwright.launch !== "function") {
    throw new TypeError("Playwright Chromium launcher is required");
  }
  const executablePath = absolutePath(chromiumExecutable, "Chromium executable");
  const origin = "http://198.18.0.1:38123";
  let browser;
  try {
    ...
    await page.route(`${origin}/**`, (route) => route.fulfill({ contentType: "text/html", body: "ok" }));
...
export function normalizeRunnerPlan(input) {
  const plan = object(input, "runner plan");
  ...
  const normalized = {
    ...
    browser: browserEngine(plan.browser),
    topology,
    profile_id: string(plan.profile_id, "profile_id"),
    run_number: positiveInteger(plan.run_number, "run_number"),
    mode,
    cold_diagnostic: optionalBoolean(plan.cold_diagnostic, "cold_diagnostic"),
...
function connectionPolicy(value, expected) {
  const policy = string(value, "policy");
  if (policy !== expected) throw new TypeError(`policy must be ${expected}`);
  return policy;
}
...
export function chromiumLaunchOptions(plan, chromiumExecutable, launcherPath, moduleOrigin) {
  ...
  const args = [`--unsafely-treat-insecure-origin-as-secure=${secureOrigin}`];
  if (normalized.diagnostics_enabled) {
    args.push(
...
export function firefoxLaunchOptions(plan, firefoxExecutable, moduleOrigin) {
  const normalized = normalizeRunnerPlan(plan);
  if (normalized.browser !== "firefox") throw new TypeError("Firefox launch requires browser=firefox");
  const executable = absolutePath(firefoxExecutable, "Firefox executable");
  const secureOrigin = browserModuleOrigin(moduleOrigin, normalized.module_advertise_host);
  if (!secureOrigin.startsWith("https://")) throw new TypeError("Firefox module origin must use HTTPS");
  return {
    headless: true,
...
function browserEngine(value) {
  const engine = value === undefined ? "chromium" : string(value, "browser");
  if (!BROWSER_ENGINES.has(engine)) throw new TypeError(`unsupported browser ${JSON.stringify(engine)}`);
  return engine;
}
...
function browserModuleOrigin(value, advertiseHost) {
  ...
  try {
    ...
    throw new TypeError("browser module origin must be an absolute HTTP(S) origin");
  }
  if ((origin.protocol !== "http:" && origin.protocol !== "https:") || origin.username !== "" || origin.password !== "" ||
    ...
    throw new TypeError("browser module origin must use the advertised IP and an explicit HTTP(S) port");
  }
