...
export function adaptNativeCarrierSessionV3(native: NativeCarrierSessionV3): CarrierSessionV3 {
  return new NativeCarrierSessionAdapter(native);
}

export function createMemoryCarrierPairV3(
  options: Readonly<{
    kind: CarrierKind;
    path: PathKind;
    inboundBidirectionalStreamCapacity: number;
...
class MemoryCarrierSession implements CarrierSessionV3 {
  readonly kind: CarrierKind;
  readonly path: PathKind;
  readonly inboundBidirectionalStreamCapacity: number;
  readonly unreliableDatagrams = undefined;

  private readonly incoming = new AsyncQueue<CarrierStreamV3>();
  ...
  enqueue(stream: CarrierStreamV3): void {
    if (this.closed) {
      void stream.reset();
      return;
    }
...
class MemoryCarrierStream implements CarrierStreamV3 {
...
class NativeCarrierSessionAdapter implements CarrierSessionV3 {
  readonly kind: "webtransport" | "raw_quic";
  readonly path: PathKind;
  readonly inboundBidirectionalStreamCapacity: number;
  readonly unreliableDatagrams: CarrierUnreliableDatagramsV3 | undefined;

  constructor(private readonly native: NativeCarrierSessionV3) {
...
class NativeCarrierStreamAdapter implements CarrierStreamV3 {
  constructor(private readonly native: NativeCarrierStreamV3) {}
