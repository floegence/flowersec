...
export class SessionV3 implements SessionV3Contract {
  ...
  async localReset(stream: EncryptedStreamV3, error: Error): Promise<void> {
    ...
    try {
      await this.sendControl(InnerTypeV3.StreamReset, idReason(stream.id, 6));
  ...
  private async resetInboundBeforeDelivery(id: bigint, carrierStream: CarrierStreamV3): Promise<void> {
    await carrierStream.reset().catch(() => undefined);
    try {
      await this.sendControl(InnerTypeV3.StreamReset, idReason(id, 3));
      ...
      while (this.lifecycle !== "closed") {
        ...
        switch (record.type) {
          case InnerTypeV3.Ping:
            await this.sendControl(InnerTypeV3.Pong, record.payload);
            break;
    ...
    const work = (async () => {
      ...
      try {
        if (!this.sentSessionClose) {
          ...
          await this.sendControl(InnerTypeV3.SessionClose, Uint8Array.of(0, 1));
  ...
  private async receiveSessionRekeyBeforeDeadline(
    ...
    await raceAbort(this.sendControl(InnerTypeV3.SessionKeyUpdateACK, payload), signal);
    throwIfAborted(signal);
  ...
  private async commitOutboundReset(id: bigint): Promise<void> {
    await this.sendControl(InnerTypeV3.StreamReset, idReason(id, 6));
