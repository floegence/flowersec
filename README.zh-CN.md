...
- **Carrier 中立路径。** Transport v2 将 WebSocket、raw QUIC 和 WebTransport 视为同等 Carrier；由精确 Runtime 能力和产品策略选择候选，不设永久主协议或回退协议。
...
上表描述 Transport v1 的跨语言能力。Transport v2 的生产网络能力以精确 Runtime Tuple 为准：

| Transport v2 能力 | Go | TypeScript | Swift | Rust |
| --- | :---: | :---: | :---: | :---: |
| WebSocket Carrier | 支持 | Browser 支持 / Node 不支持 | 不支持 | 不支持 |
| raw QUIC Carrier | 支持 | 不支持 | 不支持 | 已测试 Adapter，不对外声明 |
| WebTransport Carrier | 支持 | Browser 支持 / Node 不支持 | 不支持 | 不支持 |

Transport v2 的本地 Smoke 不等于跨语言生产签发；发布还需要真实浏览器、弱网、qlog、迁移和性能签名 Evidence。`flowersec-tunnel` CLI 和当前 Cookbook Binary 仍为 Transport v1。
