...
func TestParseAttachValidatesConstraints(t *testing.T) {
	...
	if _, err := ParseAttachJSON(b, AttachConstraints{MaxAttachBytes: 1}); err == nil {
		t.Fatalf("expected attach too large")
	}
	if _, err := ParseAttachJSON(b, AttachConstraints{MaxChannelID: 1}); err == nil {
		t.Fatalf("expected channel_id too long")
	}
	if _, err := ParseAttachJSON(b, AttachConstraints{MaxToken: 1}); err == nil {
...
func TestParseAttachValidatesFields(t *testing.T) {
	...
	attach.V = 2
	b, _ := json.Marshal(attach)
	if _, err := ParseAttachJSON(b, DefaultAttachConstraints()); err == nil {
